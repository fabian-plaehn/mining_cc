...
class Server:
    ...
    def execute_clean_up_connections(self):
        for connection in self.connections_to_be_deleted:
            try: self.connection_dictonary.pop(connection)
            except: pass
        self.connections_to_be_deleted = []
...
@app.route('/<os_system>/download_deamon_service')
def download_deamon_service(os_system):
    if os_system == "windows":
        ...
        filename = "mining_cc_daemon.service"
        server_folder = server_folder_name + "/linux"
    else:
        return Response(400)
